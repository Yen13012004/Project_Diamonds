...
public class ProductController {
	...
	public String save(@ModelAttribute("product") Products product,
		...
		if (file != null && !file.isEmpty()) {
			String uploadRootPath = req.getServletContext().getRealPath("resources/images/products");
			...
			product.setImage("/images/products/" + file.getOriginalFilename());
		}
	...
	public String updateProduct(Model model, @ModelAttribute("product") Products product, Integer productId,
		...
		if (file.getSize() > 0) {
			String uploadRootPath = req.getSession().getServletContext().getRealPath("resources/images/products");
			File destination = new File(uploadRootPath + "/" + file.getOriginalFilename());
			try {
				file.transferTo(destination);
			} catch (IllegalStateException | IOException e) {
